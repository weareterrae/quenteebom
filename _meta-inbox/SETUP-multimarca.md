...
- **ÁGUA MINDA CONFIRMADA EM PRODUÇÃO (2026-07-05):** IG comentários (pública+DM) ✅ + IG DMs ✅ + **FB comentários (resposta pública) ✅** — tudo validado com interações reais. Para o FB funcionar foi preciso, DEPOIS de publicar: re-adicionar `pages_manage_engagement` ao use case (+ Add — funciona em Standard access com a app Live, lição Leads Terrae), token com **9 permissões**, e configurar o webhook do objeto **Page** (Manage everything on your Page → Customize → Webhooks → product Page → callback+verify → subscrever `feed`) — tinha ficado por fazer (só o do IG estava). Fase 2 FB: DM privada a quem comenta + DMs Messenger (requer use case "Engage with customers on Messenger" + `pages_messaging`).
