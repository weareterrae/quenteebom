...
const BRAND_SITE   = env("BRAND_SITE", "https://quenteebom.com");
const BRAND_DESC   = env("BRAND_DESC", "padaria/pastelaria angolana"); // descrição curta da marca para o resumo semanal // site da marca (página /inbox.html de confirmação)
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET" && url.pathname.endsWith("/digest") && url.searchParams.get("key") === VERIFY_TOKEN) {
    ...
    if (amostra) {
      resumo = await claude(
        `És um analista de comunidade da marca ${BRAND} (${BRAND_DESC}). Analisas o que os clientes escreveram esta semana nas redes sociais e devolves um briefing curto e útil para a equipa. Português de Angola, direto, sem floreados. NÃO uses markdown de títulos (#); usa texto simples com quebras de linha.`,
  ...
  if (req.method === "POST") {
    ...
    for (const it of extract(payload)) {
      try {
        ...
        // Lista vermelha (5 Set): bancos, investidores, imprensa, sócios e parceiros respondem-se à mão, sempre.
        const VERMELHO = /(banc[oa]s?|banc[áa]ri[oa]s?|investidor(es|a|as)?|investimentos?|investir|jornalistas?|imprensa|jornal|reportagem|entrevista|s[óo]ci[oa]s?|sociedade|parceria|parceir[oa]s?|distribuidor(es|a|as)?|fundo de investimento|capital)/i;
        const listaVermelha = VERMELHO.test(String(it.incoming || ""));
        ...
        if (ins?.id) {
          if (autoOk && pub && !listaVermelha) {
            ...
            await notify({ ...it, id: ins.id, pub, priv, sig: await hmacHex(HMAC_SECRET, ins.id),
              holdReason: listaVermelha ? "Lista vermelha (banco, investidor, imprensa, sócio ou parceiro): rascunho feito, mas decides tu se respondes." : autoMotivo });
          }
        }
